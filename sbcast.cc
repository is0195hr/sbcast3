...
/*    if (my_addr() == 6) {
        ...
        if(ph->pkttype_==PKT_CODED) {
            fprintf(kakunin5File, "PT:CODE ID:%d (%d %d)\n", ph->pktnum_,ph->pkt1_,ph->pkt2_);
        }
        //fprintf(kakunin5File,"nowconter:%d\n",getArrayFirstEntry(downstreamNodeList,-1));
        //fprintf(kakunin5File,"sedcount:%d\n",downstreamNodeListCount);
        fprintf(kakunin5File, "%f\n", Scheduler::instance().clock());
        fprintf(kakunin5File, "UP[");
        for (int i = 0; i < upstreamNodeListCount; i++) {
            fprintf(kakunin5File, "%d ", upstreamNodeList[i]);
        }
        fprintf(kakunin5File, "]\n");
        fprintf(kakunin5File, "DW[");
        for (int i = 0; i < downstreamNodeListCount; i++) {
...
/*    float neigh_freq=0;
    ...
    float sender_freq = sender_freq=hitcount[my_addr()];
    ...
    //下流ノード数による判定するかの判定
    //if (hendou <= HENDOU) {
    if(bunpu_avg[my_addr()]>bunpu_median[my_addr()]){
//        fprintf(fp, "s\n");
        judge_res = 1;//teian=1
        switch (SIM_MODE) {
            case MODE_FL:
...
void SBAgent::printRes(){
    ...
    if(done == false && Scheduler::instance().clock() > END_TIME + ADD_TIME){
        ...
        //総パケット数
        //fprintf(resFile,"packet_count:%d %d %d %d\n",rcv_packet_count,send_packet_count,req_packet_count,rep_packet_count);
        fprintf(resFile,"%d %.1f %d %.2f %d %d %d ",SIM_MODE,TIME_TH,NEIGHBOR_TH,HENDOU,send_n_packet_count,send_c_packet_count,send_packet_count);
...
int SBAgent::forwardCodePacket(Packet * bp) {
    ...
    struct hdr_beacon * ph = HDR_BEACON(bp);
